...
The simplest method, which you already uncommented involves setting a 
threshold and detecting any value that exceeds this threshold as a peak.
This method is easy to implement but may not be the most accurate for 
detecting precise systolic and diastolic peaks.
...
A moving average filter smoothens the signal and helps in reducing noise. 
Peaks can be detected more reliably after filtering.
*/
...
/*3. Derivative Method
This method involves calculating the first derivative of the signal and 
identifying points where the derivative changes from positive to negative, 
which corresponds to a peak.
*/
...
Adaptive thresholding adjusts the threshold based on the signal
 characteristics over time, which can be more robust in dynamic environments.
*/
...
void detectPeakWaveletTransform(int sensorValue) {
  ...
  for (int i = 1; i < wt->outlength - 1; i++) {
    if (wt->output[i] > peakThreshold && wt->output[i] > wt->output[i - 1] && wt->output[i] > wt->output[i + 1] && (i - lastPeakIndex) > minPeakDistance) {
      lastPeakIndex = i;
      return wt->output[i];
    }
  }
  return -1; // No peak detected
}

/*5. Wavelet Transform
Wavelet transforms can be used to detect peaks by analyzing the signal
 at multiple scales. This method is more complex but can be very effective.
*/
